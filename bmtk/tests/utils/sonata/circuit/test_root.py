...
"""
from bmtk.utils.sonata.circuit.file import File
...
def test_save_gids(thalamocortical):
    ...
    assert(set(np.unique(gid_h5['population'][...])) == {0, 1})
    assert(set(np.unique(gid_h5['population_names'][...])) == {'v1', 'lgn'})


def test_nodes(thalamocortical):
    nodes = thalamocortical.nodes
    assert(nodes.root_name == 'nodes')
    assert(set(nodes.population_names) == {'lgn', 'v1'})
    assert('v1' in nodes)
    assert('lgn' in nodes)
